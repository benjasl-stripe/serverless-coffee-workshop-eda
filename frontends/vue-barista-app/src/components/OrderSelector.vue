...
console.log('token::::',jwtToken)
      ...
      try {
        ...
        open.data.map((order) => {
          ...
          if (order.orderNumber) {
            console.log('Open orders:', order)
            let drinkOrder =  JSON.parse(order.drinkOrder)
             console.log('Open orders:', drinkOrder)
            newOrder = {
              orderId: order.SK,
    ...
    async orderComplete (order) {
      ...
      try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$APIurl}/orders/${order.orderId}?action=complete`,
          headers: {
    ...
    async orderCancel (orderId) {
      ...
      try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$APIurl}/orders/${orderId}?action=cancel`,
          headers: {
            Authorization: 'Bearer ' + jwtToken
          }
        })
    ...
    async makeOrderClick (orderId, action) {
      ...
      try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$APIurl}/orders/${orderId}?action=${action}`,
          headers: {
            Authorization: 'Bearer ' + jwtToken
          }
        })
